...
if ! [ -d sources/meta-ntel ] ; then
	echo "Metadata checkout missing"
	if [ -e ${SHARED_DL_DIR}/../${NTELVERSION}-gits.tar.xz ] ; then
		echo "Extracting metadata cache"
		xz -T0 -dkc  ${SHARED_DL_DIR}/../${NTELVERSION}-gits.tar.xz | tar x
	fi
fi
